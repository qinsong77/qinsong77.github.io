...
> 中序遍历 从小到大， 即先访问左侧节点，访问节点本身，再访问右侧节点;中序遍历的一种应用就是对树进行排序操作。
>
> 先序遍历 先访问本身，在访问左侧节点，再访问右侧节点;先序遍历的一种应用是打印一个结构化的文档。
>
> 后续遍历 从大到小，即先访问右侧节点，访问节点本身，再访问左侧侧节点;后序遍历的一种应用是计算一个目录及其子目录中所有文件所占空间的大小。
```javascript
class Node {
	constructor(key) {
		this.key = key
