...
单项数据流：
组件相当于一个函数，props 相当于函数的传参。如果组件内部可以改变 props 就相当于，在函数内部改变参数。那么这个函数就产生了副作用，那么这个函数就不是一个 `pure function`。这会使函数变的不可测试，不可测试也就不能预测执行结果，从而降低代码可维护性。
...
- [react16-面试题](https://github.com/semlinker/reactjs-interview-questions)
...
即如Input的value绑定了state，而`onChange`时使用`setState`更新。
...
其中`FiberRootNode`是整个应用的根节点，`RootFiber`是调用`ReactDOM.render`创建的fiber。
...
- 提高了开发效率而非性能，Virtual Dom的优势在于 React的 Diff算法和批处理策略，在重复渲染时它帮助我们计算如何更高效的更新，而不是它比 DOM操作更快。
...
如何将传统`O(n^3)`diff算法的时间复杂度降为`O(n)`
...
1. 先判断`key`是否相同（**props没有key值是`null`**，实验打印出Jsx中返回的React Element key是`null`）
...
从上述代码中可以看出，对于最初 `React.lazy()` 所返回的 `LazyComponent` 对象，其 `_status` 默认是 `-1`，所以首次渲染时，会进入 `readLazyComponentType` 函数中的 default 的逻辑，这里才会真正异步执行 `import(url)`操作，由于并未等待，随后会检查模块是否 Resolved，如果已经Resolved了（已经加载完毕）则直接返回`moduleObject.default`（动态加载的模块的默认导出），否则将通过 throw 将 thenable 抛出到上层。
...
- 实现多版本共存: 事件委托从attach到`document`的代理，变成了事件 `attach` 到 React 渲染树的根 `DOM` 容器中
- 新的优先级算法 - `lanes`
...
re-render的最本质原因其实都是始于`state`状态的变化，组件`state`变化，下面的所有组件都会re-render，除非用 `React.memo`/`shouldComponentUpdate` 主动跳过）。

还有就是`Context`变化，真正用 `useContext` / `Consumer` 订阅了该 `Context` 的组件才会重渲染；

但有两个常见“看起来都渲染了”的场景：

- 父组件因为 自己订阅了该 `Context` 而重渲染，于是它的所有子组件会跟着走一次“级联渲染”（除非子组件用 `React.memo`/`shouldComponentUpdate` 跳过）。
- `Provider` 本身所在组件 自身 `state` 改变 导致整棵子树重渲染——这是普通的 `state-driven render`，不是 `Context` 直接触发的。
---------
...
const MemoStaticInfo = React.memo(StaticInfo)
...
export default App
```
点击button，输出：`2 3 4`。即`App`和`MemoStaticInfo`不会re-render。
...
注意：如果子组件被 `React.memo` 包装，那么它的所有非基础数据类型的 props 也要做 memoize 处理(即`useMemo` and `useCallback`)。

- Move state down - 状态下放

![](./image/re-render-move-state-down.png)

在 React 中，组件本质上就是函数，函数有单一职责原则，组件也适用

- children as props - children 作为 props

或者直接组件作为 props，即Lift content up，内容提升，从state变化的上层传递下来。

![](./image/re-render-children-as-props.png)
...
但如果是只改变对象的值，点击并不会引起 `re-render`，eg:
```js
...
| 目标               | 解释             |
|------------------|----------------|
| 1. 减少 **渲染次数**   | 避免不必要的组件重新渲染   |
| 2. 降低 **单次渲染成本** | 让必须发生的渲染更快、更轻量 |

### 触发重新渲染的四种场景
1. **根节点 `setState` / dispatch** → React 默认自顶向下整棵树重新渲染
2. **组件自身 state / reducer 变化**
3. **父组件渲染，但传给子组件的 props 未变**（可跳过）
4. **父组件渲染，且传给子组件的 props 已变**（必须重新渲染）


### 减少不必要渲染的完整手段
| 技术/策略                     | 适用场景 & 备注                                         |
|---------------------------|---------------------------------------------------|
| **React.memo**            | 函数组件，默认浅比较，可自定义 compare                           |
| **useMemo / useCallback** | 缓存值或函数引用，切断子组件 props 变化                           |
| **key 稳定且唯一**             | 列表渲染时避免节点错位导致的重复渲染                                |
| **状态位置优化**                | 状态向下放 / 向上提，减少无关订阅                                |
| **类组件**                   | `PureComponent`（浅比较）或手写 `shouldComponentUpdate`   |
| **代码分割**                  | `React.lazy` + `Suspense` / `loadable-components` |
| **列表虚拟化**                 | `react-window` / `react-virtualized`              |
| **immutable 数据**          | 配合浅比较更高效                                          |
| **React Compiler (实验)**   | 自动 memoization                                    |

### 降低单次渲染成本
| 技术/策略                         | 说明             |
|-------------------------------|----------------|
| **useMemo / reselect**        | 缓存高开销计算        |
| **避免 render 内匿名函数/对象**        | 防止每次生成新引用      |
| **组件拆分**                      | 降低单次 diff 节点数  |
| **SSR / SSG / Streaming SSR** | 首屏渲染前移         |
| **startTransition**           | 把非紧急更新标记为可中断   |
| **useDeferredValue**          | 让高开销组件延迟到空闲时更新 |
| **Suspense + lazy**           | 边界加载，减少一次性渲染量  |

---

### ✅ 一句话总结
> “减少渲染次数”靠 **memo、缓存、状态粒度控制**；“降低渲染成本”靠 **缓存、拆分、异步、虚拟化、SSR** 等组合拳。
