...
- **仅展示**：可用 `toFixed` 或 `toPrecision` 选择所需精度，再用 `parseFloat` 转成浮点数。注意这两个方法遵循 IEEE 754 舍入标准（舍入至最接近的值），并非严格的四舍五入。
- **参与运算**：将小数转成整数再运算。只要运算过程中的数字不超过 `Number.MAX_SAFE_INTEGER`，结果就是精确的。
- **推荐**：使用成熟的第三方库（见下文）。

#### `toFixed` 的问题

- 参考：[为什么(2.55).toFixed(1)等于2.5？](https://www.cnblogs.com/zhangycun/p/7880580.html)

`toFixed` 的结果在某些场景下会出现反直觉的 bug：

```javascript
1.54.toFixed(1) // '1.5'
1.55.toFixed(1) // '1.6'
1.64.toFixed(1) // '1.6'
1.65.toFixed(1) // '1.6'  ← 预期 '1.7'，但实际是 '1.6'
1.66.toFixed(1) // '1.7'
```

根本原因有两点：

1. **浮点数存储偏差**：小数在内存中实际存储的值与字面值不同。例如 `1.65` 实际存储为 `1.64999999999999991118...`，所以 `toFixed(1)` 的结果是 `1.6`。
2. **银行家舍入法（Round Half to Even）**：`toFixed` 规范上采用"四舍六入五考虑"——五后非零就进一，五后为零则看奇偶（五前为偶舍去，五前为奇进一）。但由于第一点的影响，实际行为因引擎而异，结果并不可靠。

下表展示了 `toFixed(2)` 的具体表现：

| 输入值 | 预期结果（四舍五入） | `toFixed(2)` 实际结果 | 原因分析 |
|--------|---------------------|----------------------|---------|
| 1.335  | 1.34                | 1.33                 | 实际存储为 `1.33499...`，向下舍入 |
| 1.345  | 1.35                | 1.35                 | 实际存储为 `1.345000...0002`，向上进位 |
| 1.355  | 1.36                | 1.35                 | 实际存储为 `1.35499...`，向下舍入 |
| 1.105  | 1.11                | 1.10                 | 浮点数表示偏差，实际略小于 1.105 |
| 2.445  | 2.45                | 2.44                 | 浮点数表示偏差，实际略小于 2.445 |

#### `Number.EPSILON` 的局限性

`Number.EPSILON` 表示 1 与大于 1 的最小浮点数之间的差值，约为 $2^{-52} \approx 2.22 \times 10^{-16}$。
...
网上流传的 `Math.round((num + Number.EPSILON) * 100) / 100` 方案**并不严谨**，本质上是"撞大运"的写法，原因如下：

- **量级不匹配**：`Number.EPSILON` 极小（约 $2.22 \times 10^{-16}$）。若数字较大（如 `1000000.335`），`1000000.335 + Number.EPSILON` 因浮点精度限制，那个微小的修正量会直接被"吞掉"，毫无效果。
- **双重舍入风险**：在极少数情况下，加上该值反而会使原本处于中间值的数跳入下一区间，导致结果错误。

#### 推荐方案

**方案一：科学计数法位移（轻量可靠）**

利用字符串形式的科学计数法来精确移动小数点，避免浮点乘法引入新的误差：
...
export function roundToDecimals(value: number, decimals: number = 2): number {
  ...
  // "1.335e2" → 133.5 → Math.round → 134 → "134e-2" → 1.34
  const rounded = Math.round(Number(`${absValue}e${decimals}`));
  const result = Number(`${rounded}e-${decimals}`);

  return sign * result;
}

console.log(roundToDecimals(1.335, 2)); // 1.34 ✓
console.log(roundToDecimals(1.105, 2)); // 1.11 ✓
console.log(roundToDecimals(2.445, 2)); // 2.45 ✓
```

**方案二：使用成熟的库（最稳妥，推荐用于货币场景）**
...
new Big(1.335).toFixed(2); // "1.34" ✓
new Big(1.105).toFixed(2); // "1.11" ✓
new Big(2.445).toFixed(2); // "2.45" ✓
```

**方案三：`Intl.NumberFormat`（仅展示场景）**
...
console.log(formatNumber(0.065, 2)); // "0.07"
console.log(formatNumber(1.335, 2)); // "1.34"
```

> 注意：`Intl.NumberFormat` 适合格式化展示，不适合需要继续参与运算的场景。
